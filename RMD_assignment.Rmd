...
ggplot(data=gapminder, aes(x=lifeExp, y=gdpPercap))+
  ...
  geom_text(aes(x=40, y=30000, label=(paste('The r-squared is ' , round(summary(obj)$r.squared,digits=3))),size=3))
```
...
plottable<-ddply(gapminder,c("continent",'year'),summarise, mle=mean(lifeExp))
pplottable<-ddply(plottable,c("continent"),summarise, up=max(mle), down=min(mle))
ggplot()+geom_point(data=plottable, aes(x=continent,y=mle,color=year), size=3) + 
  geom_linerange(data=pplottable, aes(continent,ymin=down,ymax=up), color=c('orange','orange','orange','orange','orange'), size=c(1,1,1,1,1)) +
  geom_text(data=pplottable, aes(continent, y=(pplottable$up+pplottable$down)/2, label=(round((pplottable$up-pplottable$down),digits=1))), hjust=1.5) +
...
plottable<-ddply(gapminder,c("continent",'year'),summarise, gdp=mean(gdpPercap))
pplottable<-ddply(plottable,c("continent"),summarise, up=max(gdp), down=min(gdp))
ggplot()+geom_point(data=plottable, aes(x=continent,y=gdp,color=year), size=3) + 
  geom_linerange(data=pplottable, aes(continent,ymin=down,ymax=up), color=c('orange','orange','orange','orange','orange'), size=c(1,1,1,1,1)) +
  geom_text(data=pplottable, aes(continent, y=(pplottable$up+pplottable$down)/2, label=format((round((pplottable$up-pplottable$down),digits=1)),big.mark=',', scientific = FALSE), hjust=1.25)) +
  labs(y='Mean GDP')+
