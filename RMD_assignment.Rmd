...
gapminder %>% group_by(continent,year) %>% summarize(meanlifeExp=mean(lifeExp)) %>%
  ggplot(aes(x=continent,y=meanlifeExp,color=year))+geom_point() ##add range
```
```{r plot66, echo=FALSE, fig.align='center'}
gapminder %>% group_by(continent,year) %>% summarize(gdp=mean(gdpPercap)) %>%
  ggplot(aes(x=continent,y=gdp,color=year))+geom_point() ##add range
