...
Now lets take a look at mean life expectancy by continent.
...
Further, this can be separated by year as well. 

```{r plotting101, echo=FALSE, fig.align='center', fig.width = 7, fig.height = 4}
gapminder %>% group_by(continent,year) %>% summarize(meanlifeExp=mean(lifeExp)) %>%
  ggplot(aes(x=continent,y=meanlifeExp,color=year))+geom_point()
```

It appears the life expectancy of Asia has increased the most over time, while the life expectancy in Oceania has changed the least, with the highest overall mean.
